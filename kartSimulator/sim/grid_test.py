...
class KartSim(gym.Env):
    ...
    def __init__(self, render_mode=None,
        ...
        # player stuff
        self.max_velocity = player_args["max_velocity"] * PPM / 2
        self.player_acc_rate = player_args["player_acc_rate"] / 2
        ...
        if value == 0:
            ...
            if self.velocity < MAX_VELOCITY:
                pygame.draw.circle(
                    ...
                    (255, 255, 255),
                    (self._playerBody.position.x, self._playerBody.position.y - self.bot_size * PPM * 2 - 10),
                    5,
                )
        ...
        if value == 0:
            ...
            if self.velocity < MAX_VELOCITY:
                pygame.draw.circle(
                    self._window_surface,
                    (255, 255, 255),
                    (self._playerBody.position.x - self.bot_size * PPM * 2 - 10, self._playerBody.position.y),
                    5,
                )
    ...
    def create_player(self) -> None:
        ...
        width = self.bot_size * PPM * 2  # width of the rectangle
        height = self.bot_size * PPM  * 2 # for example, taller than wide


        # create a box
        shape = pymunk.Poly.create_box(None, (width, height))  # None for now, body added later

        # calculate inertia
        inertia = pymunk.moment_for_box(mass, (width, height))
        body = pymunk.Body(mass, inertia)
        body.position = self.initial_pos
        shape.body = body  # attach body to shape
