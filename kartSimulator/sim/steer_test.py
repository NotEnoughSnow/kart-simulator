...
class KartSim(gym.Env):
    ...
    def __init__(self,
        ...
        # player stuff
        self.max_velocity = player_args["max_velocity"] * PPM * 1.141 / 2
        self.player_acc_rate = player_args["player_acc_rate"] * 1.141 / 2
        self.player_break_rate = player_args["player_break_rate"] * 1.141 / 2
        self.player_rad_velocity = player_args["rad_velocity"] * 5.803 / 2
    ...
    def render(self, mode):
        ...
        pygame.draw.circle(self._window_surface, (255, 255, 0), (300, 500), 2)
    ...
    def create_player(self) -> None:
        #pygame.draw.line(self._window_surface, (0, 255, 0), player_position, (player_x, player_y), 2)

        mass = self.bot_weight
        long_side = self.bot_size * PPM * 2  # the side you want long
        short_side = self.bot_size * PPM * 2 * 0.7  # shorter sides
        height = long_side  # just for simplicity

        # define vertices relative to center
        vertices = [
            (0, height / 2),  # top point
            (-short_side / 2, -height / 2),  # bottom left
            (short_side / 2, -height / 2)  # bottom right
        ]
