...
class Robot:
    ...
    def input_command(self, action_array):
        ...
        print("huh ?", self.pending_command)
    ...
    def update(self, body):
        ...
        x = self.input_velocity * math.cos(body.angle)
        y = self.input_velocity * math.sin(body.angle)
...
class KartSim(gym.Env):
    ...
    def __init__(self,
        ...
        # player stuff
        self.player_max_velocity = player_args["player_acc_rate"] / 2
        self.player_rad_velocity = player_args["rad_velocity"] / 4
    ...
    def reset(
        ...
        angle = math.pi + math.pi/2 + math.pi/6
        #angle = math.pi + (2 * math.pi)/3 + math.pi/3 + math.pi/3
    ...
    def execute_steps(self):
        ...
        goal_angle_1 = math.pi + math.pi/6 - 0.001
        goal_angle_2 = +(math.pi/ 4) - 1.5
        goal_angle_3 = +(math.pi/ 1) - 1.5
        goal_position_x = 400
        goal_position_y = 600

        '''
        if (self._playerBody.angle < goal_angle_1) and (self._playerBody.position.x < goal_position_x):
            self.robot.input_command([0, 1, 0, 0])
        
        if (self._playerBody.angle < goal_angle_2) and (self._playerBody.position.x > goal_position_x):
            self.robot.input_command([0, 0, 0, 1])

        if (self._playerBody.angle > goal_angle_2) and (self._playerBody.position.y < goal_position_y):
            self.robot.input_command([0, 1, 0, 0])

        if (self._playerBody.angle < goal_angle_3) and (self._playerBody.position.y > goal_position_y):
            self.robot.input_command([0, 0, 0, 1])

        if (self._playerBody.angle > goal_angle_3) and (self._playerBody.position.x > start_position_x):
            self.robot.input_command([0, 1, 0, 0])'''


        if self.robot.pending_command == None:
            self.robot.input_command([0, 0, 0, 0])

        #print(self.robot.pending_command)
        print("current angle", self._playerBody.angle)
        #print("current pos", self._playerBody.position)
        print("goal 1", goal_angle_1)
        #print("goal 2", goal_angle_2)
        #print("goal 3", goal_angle_3)

    def draw_triangle(self, segment_length=100):

        if not hasattr(self, "triangle_state"):
            self.triangle_state = "move"
            self.triangle_start_pos = (300, 500)
            self.triangle_target_angle = None

        print(self.triangle_state)

        if self.triangle_state == "move":
            dx = self._playerBody.position.x - self.triangle_start_pos[0]
            dy = self._playerBody.position.y - self.triangle_start_pos[1]
            dist = math.sqrt(dx ** 2 + dy ** 2)
            print("huh", dist)
            if dist < segment_length:
                self.robot.input_command([0, 1, 0, 0])
            else:
                self.triangle_state = "rotate"
                self.triangle_start_pos = (self._playerBody.position.x, self._playerBody.position.y)
                self.triangle_target_angle = (self._playerBody.angle + 2 * math.pi / 3) % (2 * math.pi)

        elif self.triangle_state == "rotate":
            # normalize both angles
            current_angle = self._playerBody.angle % (2 * math.pi)
            diff = (self.triangle_target_angle - current_angle) % (2 * math.pi)
            if diff > 0.05:
                self.robot.input_command([0, 0, 0, 1])
            else:
                self.triangle_state = "move"
    ...
    def step(self, action: Union[np.ndarray, int]):
        ...
        direction = (math.cos(self._playerBody.angle), math.sin(self._playerBody.angle))
        ...
        if action is not None:
            ...
            step_reward, terminated, truncated = 0, 0, 0
    ...
    def draw_angles(self, screen):
        ...
        player_x = player_position[0] + line_length * math.cos(self._playerBody.angle)
        player_y = player_position[1] + line_length * math.sin(self._playerBody.angle)
    ...
    def observation_rotation(self):
        ...
        player_angle = (self._playerBody.angle)
