...
class KartSim(gym.Env):
    ...
    def __init__(self, render_mode=None,
        ...
        act_low = np.array([-1, -1]).astype(np.float32)
        act_high = np.array([1, 1]).astype(np.float32)
    ...
    def step(self, action: Union[np.ndarray, int]):
        ...
        if action_array is not None:
            go_up_value = self._go_up_down(action_array[0])
            go_left_value = self._go_left_right(action_array[1])
    ...
    def reward_logic(self, pstart, pend, action):
        ...
        if self.next_sector_name is not None:
            ...
            self.distance_to_next_goal = self.distance(pend, self.goal_pos)
            ...
            initial_distance = -self.distance(self.goal_pos, pstart)
            final_distance = -self.distance(self.goal_pos, pend)
            self.next_target_rew_act = (final_distance - initial_distance)*speed_trans
